...
export default {
  ...
  methods: {
    async editallpricedown() {
      const uid = this.products[0].intproductId;
      await AuthenticationServices.adminupdateallProductdown({
        price: this.allprice,
        intproductId: uid,
      });
    },
    async editallprice() {
      if (this.allprice) {
        const uid = this.products[0].intproductId;
        await AuthenticationServices.adminupdateallProduct({
          price: this.allprice,
          intproductId: uid,
        });
      } else if (this.filex) {
        ...
        formData.append("image", this.filex);
        formData.append("intproductId", uid);
        await AuthenticationServices.adminupdateallProduct(formData);
      }
    },
    ...
    uploadImages() {
      this.filex = this.$refs.filex.files[0];
    },
    async adminProduct() {
      this.alerte = "";
      if (
    ...
    async updateProduct() {
      ...
      if (this.title != null) {
        ...
        await AuthenticationServices.updatedadminProduct(this.activeItem, {
          title: this.title,
        });
      } else if (this.filed != null) {
        console.log(this.filed);
        let formData = new FormData();
